...
CSV 文件格式要求:
    - 必须包含以下列: 项目名称, 一句话介绍, 项目详细介绍, 提交人
    - 可选列: Bohrium App 链接, 团队成员和单位, 开发故事, 负责人姓名, 负责人单位

更新说明:
    - 所有作品默认分类为 "general-research"（通用科研）
    - 作者信息从 CSV 的"提交人"字段提取（而非"参赛者003"格式）
    - 可手动修改每个作品的分类为：biology, medicine, architecture, physics,
      chemistry, materials, pharmacy, simulation, experiment
"""
...
def generate_mdx_from_csv(csv_file, output_dir='content/blog', cdn_base='https://cdn.example.com'):
    ...
    for i, row in enumerate(rows, 1):
        try:
            ...
            # 优先使用"提交人"字段作为项目负责人
            team_leader = row.get('提交人', '').strip() or row.get('负责人姓名', '').strip()
            team_org = row.get('负责人单位', '').strip()

            # 生成作者 slug（用于 author 字段）
            author_slug = slugify(team_leader) if team_leader else f'contestant-{work_id}'
...
def create_authors_from_csv(csv_file, output_dir='content/author'):
    ...
    # 用于去重的集合（避免同一个人创建多个作者文件）
    created_authors = set()

    for i, row in enumerate(rows, 1):
        work_id = f"{i:03d}"
        # 优先使用"提交人"字段
        team_leader = row.get('提交人', '').strip() or row.get('负责人姓名', '').strip() or f'参赛者{work_id}'

        # 生成作者 slug
        author_slug = slugify(team_leader)

        # 如果已经创建过这个作者，跳过
        if author_slug in created_authors:
            print(f"⏭️  {author_slug}.mdx (已存在)")
            continue

        filename = f"{author_slug}.mdx"
        filepath = os.path.join(output_dir, filename)
...
"""
        ...
        created_authors.add(author_slug)
        print(f"✅ {filename} ({team_leader})")

    print(f"🎉 完成！共生成 {len(created_authors)} 个作者文件")
