...
![dokploy website](https://cdn.mksaas.me/images/d1faa33175f072995d8d7b9f6bbf57ed.png)
...
![Hostinger](https://cdn.mksaas.me/images/923a97a1f701219e9547a5420fae82d3.png)
...
![install dokploy](https://cdn.mksaas.me/images/04f7211d232067f85f2f3061952b43e2.png)

### 2.3 配置防火墙

在服务器的防火墙配置中新增一条规则，允许3000端口可以被外界访问。

![dokploy setup the server](https://cdn.mksaas.me/images/f4cbdf4a1f73bc55ad69ac38f25877c4.png)
...
![dokploy bind domain](https://cdn.mksaas.me/images/d41b18392078b646b91b102fc3ac718c.png)

图片中设置的是个子域名，你也可以设置主域，设置完成后，在域名的DNS解析中添加一条记录，类型是A，将配置的域名指向服务器的IP地址。

![dns record](https://cdn.mksaas.me/images/2c24e724e45a7cc6ec4e3eb0ba01f670.png)

DNS解析配置后，我们就可以通过https+域名的方式来访问服务器上的Dokploy。

![visit dokploy with https://domain](https://cdn.mksaas.me/images/5bc42ba4b73ceb135995fb1034a9a4c7.png)
...
![setup github](https://cdn.mksaas.me/images/0f2de55c2d81ab8256c91451488753ce.png)

![create github app](https://cdn.mksaas.me/images/bc79b2cc5ebf22955d29c272c0edb0d0.png)

等Github应用创建完成之后，可以点击这个应用，进行Github授权，这样的话Dokploy就可以访问你的Github账号下的代码仓库。

![config git providers](https://cdn.mksaas.me/images/73e5ab19befc8793b387357978315f31.png)

![authorize the Github app](https://cdn.mksaas.me/images/74c110cb26cf8b5cc8edf864df5357a0.png)
...
![deploy settings for github repository](https://cdn.mksaas.me/images/80eef6e611771bef1ad285f3106ac3b1.png)

![deploy settings for build type](https://cdn.mksaas.me/images/537b03c6a8be60614428ff36ffc7a533.png)

首次构建失败了，可以查下编译日志，原因是pnpm-lock.yaml文件缺失。

![build fail logs](https://cdn.mksaas.me/images/bb4a6aa37c4108dda10d313470deef36.png)
...
![deployments](https://cdn.mksaas.me/images/23ecef885028b1ef76c51d9d1efc2b97.png)
...
![bind domain](https://cdn.mksaas.me/images/751e31c97cdaccfa563349455103e624.png)

这里我设置的是一个自定义子域名，然后在DNS解析中配置子域名指向服务器IP。

![add dns record](https://cdn.mksaas.me/images/64d104e1560b107c25b6fd5822b1ecb1.png)

等待一段时间之后，就可以通过https+域名访问这个Nextjs应用。

![visit nextjs app by https://domain](https://cdn.mksaas.me/images/dd6347d0b8cb231b7982ebbe77c0708e.png)
...
![personal access token](https://cdn.mksaas.me/images/e8d267b6ef0bca54c7c200a371c53c99.png)

#### 3.4.2 配置Registry

在Dokploy的Docker Registry配置中，新建一个外部Registry，Username是你的Github账号名，Password是上面创建的Token，URL是 `https://ghcr.io` 。

![add external registry](https://cdn.mksaas.me/images/8eedf647c88df1d0ad1102a459211a4e.png)

#### 3.4.3 重新配置部署方式

为了演示，这里我新建一个Application，这次我们换成Docker的部署方式。首先在应用的高级设置中，设置Registry是我们刚才创建的Registry。

![cluster settings](https://cdn.mksaas.me/images/3c2243ad1f7f574e191352a7504e6036.png)

然后，配置部署参数，Docker Image是 `ghcr.io/[Github账号名]/[Repository项目名]:[分支名]` ，然后保存。意思是，部署时，Dokploy去Registry中找到这个Docker镜像，把最新镜像拉取下来并部署上线。

![deploy settings](https://cdn.mksaas.me/images/92fc4acc6e00f996cfe482fe4482d2a7.png)
...
![deployment webhook url](https://cdn.mksaas.me/images/61f09698b721d2a2217b4fb7c3d239d5.png)

#### 3.4.5 测试访问

待部署成功之后，我们同样可以绑定域名，配置DNS解析，然后就可以通过https+域名进行安全地访问了。

![deployment success](https://cdn.mksaas.me/images/2c68da0a3d49d123d129c9b2892fd8fb.png)

![visit by https://domain](https://cdn.mksaas.me/images/0943e7a67b5d533446d3c8e1b9a2a49e.png)
...
![daily docker cleanup](https://cdn.mksaas.me/images/030a042cfd742e20ba455f299697a421.png)

### 4.2 服务器监控

Dokploy还自带了服务器监控的能力，你可以直接查看服务器的CPU利用率、内存利用率、磁盘空间，以及网络IO等。

![dokploy monitoring](https://cdn.mksaas.me/images/594c814a751800d88df06423ada4b56f.png)

### 4.3 数据库部署和备份

Dokploy也可以部署数据库，常见的数据库Postgres、MySQL、MongoDB都支持，部署之后，设置了外网访问端口就可以访问了。

![select a database](https://cdn.mksaas.me/images/379bf8753d61e90dccffabe019950fdd.png)

在External Port中设置一个端口号，然后保存就可以得到一个DB URL，只要防火墙配置中允许访问服务器的这个端口号，那么就可以连接数据库了。

![external host](https://cdn.mksaas.me/images/a73e96e35da174a03e06546ec302fa80.png)

下面用免费的Navicat Premium Lite配置好数据库连接信息测试连接没问题。

![navicat premium lite](https://cdn.mksaas.me/images/6835e67c51eb1e34b8f93a08883cdecb.png)

Dokploy还支持备份数据库，可以在S3 Destinations中配置AWS S3或者Cloudflare R2，然后将它作为目标备份点添加到这里，最后在Database的备份配置中选择这个备份点就可以实现数据库备份了。

![s3 destinations](https://cdn.mksaas.me/images/aa32c75bacdd76b0658122abbdd2eade.png)
...
![templates](https://cdn.mksaas.me/images/91a02bf3da9711fe58192e5aabb006bd.png)
